...
def profile_install(service_provider: LockdownServiceProvider, keybag: Optional[str], profiles: list) -> None:
    """
...
@click.argument('profiles', nargs=-1, type=click.File('rb'))
def profile_store(service_provider: LockdownServiceProvider, profiles: list) -> None:
    """ Store a profile """
    service = MobileConfigService(lockdown=service_provider)
    for profile in profiles:
