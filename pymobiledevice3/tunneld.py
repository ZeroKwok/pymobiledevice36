...
async def async_get_tunneld_devices(tunneld_address: tuple = TUNNELD_DEFAULT_ADDRESS) \
        -> list:
    tunnels = _list_tunnels(tunneld_address)
    return await _create_rsds_from_tunnels(tunnels)


def get_tunneld_devices(tunneld_address: tuple = TUNNELD_DEFAULT_ADDRESS) \
        -> list:
    return get_asyncio_loop().run_until_complete(async_get_tunneld_devices(tunneld_address))


async def async_get_tunneld_device_by_udid(udid: str, tunneld_address: tuple = TUNNELD_DEFAULT_ADDRESS) \
...
def get_tunneld_device_by_udid(udid: str, tunneld_address: tuple = TUNNELD_DEFAULT_ADDRESS) \
        -> Optional[RemoteServiceDiscoveryService]:
    return get_asyncio_loop().run_until_complete(async_get_tunneld_device_by_udid(udid, tunneld_address))


def _list_tunnels(tunneld_address: tuple = TUNNELD_DEFAULT_ADDRESS) -> dict:
    try:
...
async def _create_rsds_from_tunnels(tunnels: dict) -> list:
    rsds = []
